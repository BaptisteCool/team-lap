...
export const Route = createFileRoute('/event/$eventSlug/team/$teamId')({
...
function EventTeamPage() {
  const { eventSlug, teamId } = Route.useParams()
  ...
  const setActiveTab = (next: Tab) => {
    ...
    navigate({ to: '/event/$eventSlug/team/$teamId', params: { eventSlug, teamId }, search: { tab: safe, readonly } })
  }
  ...
  const event = useQuery('events:getBySlug' as any, { slug: eventSlug })
  ...
  // screen is open. See routes/event.$eventSlug.admin.tsx for the same pattern.
  useEffect(() => {
    if (!event?._id) return
    const sessionId = (() => {
      try {
      ...
      <div className="grid" style={{ gap: 14, maxWidth: 1280, margin: '0 auto' }}>
        <div className="card" style={{ padding: '8px 12px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
            <button
              className="btn ghost"
              onClick={() => navigate({ to: isAdminUnlocked ? '/event/$eventSlug/admin' : '/event/$eventSlug', params: { eventSlug } })}
              style={{ fontSize: 13 }}
            ...
            {!readonly && (
              <button
                ...
                onClick={() => {
                  ...
                  navigate({ to: '/event/$eventSlug', params: { eventSlug } })
                }}
              >
                Verrouiller
              </button>
          ...
          <PlanningScreen
            ...
            currentRunnerExpectedLapMs={(() => {
              ...
              if (cR.liveKmMin != null && cR.liveKmSec != null) {
                const liveMs = (cR.liveKmMin * 60 + cR.liveKmSec) * 900
