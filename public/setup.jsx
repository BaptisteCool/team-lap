// Setup screen — team & runners CRUD
function SetupScreen({ team, setTeam, runners, setRunners, onContinue, categories = ['Hommes', 'Mixte', 'Femmes'], adminContact = { email: '', phone: '' } }) {
  const { Icon, GpxMap, EnergySegment, StatusSegment } = window.UI;
  const { LAP_DISTANCE_M, fmtPace, kmPaceToLapMs, fmtKmPace, fmtLap, RUNNER_PALETTE, DEFAULT_PLANNED_LAPS } = window.RACE_DATA;
  ...
  function addRunner() {
    ...
    setRunners(rs => [...rs, {
      id, name: '', kmMin: 5, kmSec: 0, color: palette[rs.length % palette.length],
      energy: 100, status: 'ready', liveKmMin: null, liveKmSec: null,
  ...
  function removeRunner(id) {
    ...
    if (!window.confirm(`Supprimer ${name} de l'équipe ?\n\nSes éventuels tours déjà enregistrés resteront dans l'historique mais ne pourront plus être réattribués automatiquement.`)) return;
      ...
      <div className="grid setup-grid">
        ...
        <div className="grid" style={{ gap: 18, alignContent: 'start' }}>
          <div className="card" style={{ borderLeft: `4px solid ${team.color || 'var(--accent)'}` }}>
            <div className="card-head">
              ...
              <span className="badge" style={{ marginLeft: 'auto', fontSize: 10 }}>Lecture seule · réglé en admin</span>
            </div>
            <div className="card-body grid" style={{ gap: 14 }}>
              ...
              <div className="grid" style={{ gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
                <div className="field">
                  ...
                  <div className="mono" style={{ padding: '9px 10px', background: 'var(--bg-2)', border: '1px solid var(--border)', borderRadius: 10, fontSize: 14 }}>
                    {team.category || '—'}
                  </div>
                </div>
                <div className="field">
            ...
            <div className="card-body">
              ...
              <div className="stat-row" style={{ marginTop: 14 }}>
                ...
                <div className="stat">
                  ...
                  <div className="stat-value mono">{projectedLaps} t · {projectedKm} km</div>
                </div>
              </div>
        ...
        <div className="card" style={{ alignSelf: 'start' }}>
          <div className="card-head">
            ...
            <button className="btn primary" style={{ marginLeft: 'auto' }} onClick={addRunner} disabled={atCap}
          ...
          <div className="card-body">
            {runners.length === 0 && (
              <div className="empty">Ajoutez au moins 2 coureurs pour démarrer.</div>
            )}
            {atCap && (
              ...
              {runners.map(r => {
                ...
                return (
                  <div key={r.id} className="card" style={{
                    background: 'var(--bg-2)', borderRadius: 12, opacity: r.status === 'out' ? 0.55 : 1,
                  }}>
                    <div style={{ padding: 12, display: 'grid', gridTemplateColumns: '28px 1fr 36px', gap: 10, alignItems: 'center' }}>
                      <input type="color" value={r.color}
                             ...
                             style={{ width: 24, height: 24, padding: 0, background: r.color, border: '1px solid rgba(255,255,255,0.06)', borderRadius: 6, cursor: 'pointer' }} />
                      <input value={r.name} onChange={e => updateRunner(r.id, { name: e.target.value })}
                    ...
                    <div style={{ padding: '0 12px 12px', display: 'grid', gridTemplateColumns: '180px 110px 1fr', gap: 14 }}>
                      <div className="field">
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                          <input type="number" min="0" max="20" value={r.kmMin}
                                 onChange={e => updateRunner(r.id, { kmMin: Math.max(0, +e.target.value || 0) })}
                                 style={{ width: 56, textAlign: 'center' }} className="mono" />
                          <span style={{ color: 'var(--muted)' }}>:</span>
                          <input type="number" min="0" max="59" value={r.kmSec}
                      ...
                      <div className="field">
                        ...
                        <input type="number" min="1" max="50" value={r.plannedLaps ?? 1}
                               onChange={e => updateRunner(r.id, { plannedLaps: Math.max(1, +e.target.value || 1) })}
                               style={{ width: 80, textAlign: 'center' }} className="mono" />
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        <div>
                          <span className="field-label" style={{ display: 'block', marginBottom: 4 }}>Énergie</span>
                          <EnergySegment value={r.energy} onChange={(v) => updateRunner(r.id, { energy: v })} />
                        </div>
                        <div>
                          <span className="field-label" style={{ display: 'block', marginBottom: 4 }}>Statut de course</span>
