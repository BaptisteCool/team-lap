// Admin dashboard — global schedule, race control, interruptions, teams CRUD
const { useState: useStateAd, useEffect: useEffectAd, useMemo: useMemoAd } = React;

function AdminScreen({
  admin, setAdmin,
  teamsById,
  addTeam, updateTeamInfo, removeTeam,
  startRaceNow, startRaceAtScheduled, stopRace, correctActualStart, resetRace,
  pushToast,
  categories = ['Hommes', 'Mixte', 'Femmes'],
  ...
  // Admin password change (gated by super-PIN)
  const [pwNew, setPwNew] = useStateAd('');
  const [pwSuper, setPwSuper] = useStateAd('');
  const [pwErr, setPwErr] = useStateAd('');
  function submitPasswordChange() {
    setPwErr('');
    if (!pwNew || pwNew.length < 4) { setPwErr('Mot de passe trop court (4 caractères min).'); return; }
    if (pwSuper !== superAdminPin) { setPwErr('PIN super-admin incorrect.'); return; }
    if (!window.confirm(`Changer le mot de passe administrateur ?\n\nNouveau mot de passe : "${pwNew}"\n\nTous les administrateurs devront utiliser ce nouveau mot de passe.`)) return;
  ...
  const activeRaceItr = interruptions.find(i => i.kind === 'race' && !i.endMs);
  const activeAppItr  = interruptions.find(i => i.kind === 'app'  && !i.endMs);

  function startItr(kind, reason) {
    const id = 'i' + Date.now().toString(36);
    setInterruptions(arr => [...(arr || []), { id, kind, reason: reason || (kind === 'race' ? 'Interruption course' : 'Incident technique'), startMs: Date.now(), endMs: null }]);
  ...
  function endItr(id) {
    setInterruptions(arr => (arr || []).map(i => i.id === id ? { ...i, endMs: Date.now() } : i));
    const it = interruptions.find(i => i.id === id);
  ...
  function patchItr(id, patch) {
    setInterruptions(arr => (arr || []).map(i => i.id === id ? { ...i, ...patch } : i));
  }
  function removeItr(id) {
    setInterruptions(arr => (arr || []).filter(i => i.id !== id));
  }

  const totalRacePauseMs = interruptions
    .filter(i => i.kind === 'race')
    .reduce((a, i) => a + ((i.endMs || now) - i.startMs), 0);
        ...
        <div className="grid" style={{ gap: 18, alignContent: 'start' }}>
          <div className="card">
            <div className="card-head">
              ...
              <span className={`badge ${raceStatus === 'running' ? 'accent' : raceStatus === 'interrupted' ? 'danger' : raceStatus === 'paused' ? 'warn' : ''}`} style={{ marginLeft: 'auto' }}>
                {raceStatus === 'running' ? 'En course' : raceStatus === 'interrupted' ? 'Interrompue' : raceStatus === 'paused' ? 'En pause' : 'Programmée'}
              </span>
            </div>
            <div className="card-body grid" style={{ gap: 14 }}>
              <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div className="field">
                  ...
                  <input type="datetime-local" value={schedule.startISO || ''}
                         onChange={e => setSchedule(s => ({ ...s, startISO: e.target.value }))} />
                </div>
                <div className="field">
                  <span className="field-label">Arrivée programmée</span>
                  <input type="datetime-local" value={schedule.endISO || ''}
                         onChange={e => setSchedule(s => ({ ...s, endISO: e.target.value }))} />
                </div>
              </div>
              ...
              <div className="grid" style={{ gap: 10 }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  <button className="btn primary" onClick={startRaceNow} disabled={race.started}>
                    <Icon name="Rocket" size={16} /> Lancer maintenant
                  </button>
                  <button className="btn" onClick={startRaceAtScheduled} disabled={race.started || !scheduledStartMs}>
                    <Icon name="Calendar" size={16} /> Caler sur l'heure programmée
                  </button>
                  {race.started && (
                    <button className="btn danger" onClick={stopRace}>
                      <Icon name="Square" size={14} /> Stopper
                    </button>
                  )}
                  {(race.startTime || totalLaps > 0) && resetRace && (
                    <button className="btn ghost" onClick={resetRace} title="Effacer le départ, les tours de toutes les équipes et les interruptions">
                      <Icon name="RefreshCcw" size={14} /> Réinitialiser la course
                    </button>
                  )}
                </div>
                {!race.started && (
                  <div className="hint" style={{
                    ...
                    <span>
                      <span className="mono" style={{ color: readyTeamsCount === teamsArr.length && teamsArr.length > 0 ? 'var(--accent)' : 'var(--text)', fontWeight: 600 }}>
                        {readyTeamsCount}
                      </span>
                      {' / '}<span className="mono">{teamsArr.length}</span> équipe{teamsArr.length > 1 ? 's' : ''} prête{readyTeamsCount > 1 ? 's' : ''} au départ
                    </span>
                  </div>
                )}
                ...
                {race.startTime && (
                  <div className="card" style={{ background: 'var(--bg-2)', padding: 12, marginTop: 4 }}>
                    <div className="field-label" style={{ marginBottom: 6 }}>
                      Heure de départ effective <span style={{ color: 'var(--muted)', textTransform: 'none', letterSpacing: 0 }}>· modifiable si oubli</span>
                    </div>
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                      ...
                      <button className="btn ghost" onClick={() => correctActualStart(toLocalDatetime(scheduledStartMs))} disabled={!scheduledStartMs}>
                        <Icon name="Calendar" size={14} /> Recaler sur prévu
                      </button>
                    </div>
                    <div className="hint" style={{ marginTop: 6 }}>
                      Si vous avez oublié de cliquer "Lancer", saisissez ici l'heure réelle où la course a commencé. Tous les temps de tour seront recalculés à partir de cette référence.
                    </div>
                  </div>
                )}
              </div>
          ...
          <div className="card">
            ...
            <div className="card-body grid" style={{ gap: 12 }}>
              <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <div className="card" style={{ background: activeRaceItr ? 'oklch(0.72 0.21 25 / 0.08)' : 'var(--bg-2)', padding: 12, border: '1px solid ' + (activeRaceItr ? 'oklch(0.72 0.21 25 / 0.4)' : 'var(--border)') }}>
                  <div className="field-label" style={{ marginBottom: 6 }}>
                    <Icon name="Flag" size={11} style={{ verticalAlign: -1, marginRight: 4 }}/> Course
                  </div>
                  {activeRaceItr ? (
                    <>
                      ...
                      <button className="btn primary" onClick={() => endItr(activeRaceItr.id)}>
                        <Icon name="Play" size={14}/> Reprendre la course
                      </button>
                    </>
                  ) : (
                    <>
                      <div className="hint" style={{ marginBottom: 8 }}>
                        Drapeau rouge / accident / météo. Met la course en pause et journalise la durée.
                      </div>
                      <button className="btn danger" onClick={() => startItr('race', prompt("Motif de l'interruption ?", 'Drapeau rouge') || 'Interruption course')} disabled={!race.startTime}>
                        <Icon name="Pause" size={14}/> Interrompre la course
                      </button>
                    </>
                  )}
                </div>

                <div className="card" style={{ background: activeAppItr ? 'oklch(0.78 0.18 80 / 0.08)' : 'var(--bg-2)', padding: 12, border: '1px solid ' + (activeAppItr ? 'oklch(0.78 0.18 80 / 0.4)' : 'var(--border)') }}>
                  <div className="field-label" style={{ marginBottom: 6 }}>
                    <Icon name="Wifi" size={11} style={{ verticalAlign: -1, marginRight: 4 }}/> Service applicatif
                  </div>
                  {activeAppItr ? (
                    <>
                      ...
                      <button className="btn primary" onClick={() => endItr(activeAppItr.id)}>
                        <Icon name="Play" size={14}/> Service rétabli
                      </button>
                    </>
                  ) : (
                    <>
                      <div className="hint" style={{ marginBottom: 8 }}>
                        Bug, perte réseau, batterie. La course continue mais les saisies sont à recaler manuellement.
                      </div>
                      <button className="btn" style={{ background: 'oklch(0.78 0.18 80 / 0.15)', color: 'oklch(0.86 0.18 80)' }}
                              onClick={() => startItr('app', prompt("Type d'incident ?", 'Perte réseau') || 'Incident technique')}>
                        <Icon name="WifiOff" size={14}/> Signaler un incident
                      </button>
                    </>
                  )}
                </div>
              ...
              {interruptions.length > 0 && (
                <div>
                  <div className="field-label" style={{ marginBottom: 6 }}>Journal · {interruptions.length} entrée{interruptions.length > 1 ? 's' : ''}</div>
                  <div className="grid" style={{ gap: 4 }}>
                    {interruptions.slice().reverse().map(it => {
                      const dur = (it.endMs || now) - it.startMs;
                      const ongoing = !it.endMs;
                      return (
                        <div key={it.id} style={{
                          display: 'grid', gridTemplateColumns: '70px 1fr 110px 110px 90px 28px',
                          gap: 8, alignItems: 'center', padding: '6px 8px', borderRadius: 6,
                          background: ongoing
                            ? (it.kind === 'race' ? 'oklch(0.72 0.21 25 / 0.06)' : 'oklch(0.78 0.18 80 / 0.06)')
                            : 'var(--bg-2)',
                          border: '1px solid ' + (ongoing ? (it.kind === 'race' ? 'oklch(0.72 0.21 25 / 0.3)' : 'oklch(0.78 0.18 80 / 0.3)') : 'var(--border)'),
                          fontSize: 12,
                        }}>
                          ...
                          }}>{it.kind === 'race' ? 'COURSE' : 'SERVICE'}</span>
                          <input value={it.reason} onChange={e => patchItr(it.id, { reason: e.target.value })}
                                 style={{ background: 'transparent', border: 'none', padding: 2, fontSize: 12, color: 'var(--text)' }} />
                          <input type="datetime-local" value={toLocalDatetime(it.startMs)}
                                 onChange={e => { const t = new Date(e.target.value).getTime(); if (!isNaN(t)) patchItr(it.id, { startMs: t }); }}
                                 style={{ padding: '2px 4px', fontSize: 11 }} className="mono" />
                          <input type="datetime-local" value={it.endMs ? toLocalDatetime(it.endMs) : ''}
                                 placeholder="—"
                                 onChange={e => { const t = e.target.value ? new Date(e.target.value).getTime() : null; patchItr(it.id, { endMs: t }); }}
                                 style={{ padding: '2px 4px', fontSize: 11, opacity: ongoing ? 0.5 : 1 }} className="mono" />
                          <span className="mono" style={{ color: ongoing ? (it.kind === 'race' ? 'oklch(0.85 0.16 25)' : 'oklch(0.88 0.15 80)') : 'var(--text-2)', fontWeight: 600, textAlign: 'right' }}>
                            {ongoing ? '● ' : ''}{fmtClock(dur)}
                          </span>
              ...
              <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                ...
                <div className="field">
                  ...
                  <input type="tel" value={contact.phone || ''}
                         ...
                         placeholder="+33 6 12 34 56 78" autoComplete="off" />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="card">
            ...
            <div className="card-body grid" style={{ gap: 12 }}>
              <div className="hint">
                Mot de passe partagé entre plusieurs admins. Le changement requiert le PIN super-admin (réservé à l'admin principal).
              </div>
              <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <div className="field">
                  <span className="field-label">Nouveau mot de passe admin</span>
                  <input type="text" value={pwNew} onChange={e => setPwNew(e.target.value)}
                         placeholder="nouveau mot de passe" autoComplete="off" />
                </div>
                <div className="field">
                  <span className="field-label">PIN super-admin</span>
                  <input type="password" inputMode="numeric" maxLength={8} value={pwSuper}
                         onChange={e => setPwSuper(e.target.value.replace(/\D/g, ''))}
                         className="mono" style={{ letterSpacing: '0.3em', textAlign: 'center' }}
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button className="btn primary" onClick={submitPasswordChange} disabled={!pwNew || !pwSuper}>
                  <Icon name="KeyRound" size={14} /> Changer le mot de passe
                </button>
                {pwNew || pwSuper ? (
                  <button className="btn ghost" onClick={() => { setPwNew(''); setPwSuper(''); setPwErr(''); }}>
                    Annuler
                  </button>
                ) : null}
              </div>
        ...
        <div className="card" style={{ alignSelf: 'start' }}>
          <div className="card-head">
            ...
            <button className="btn primary" style={{ marginLeft: 'auto' }} onClick={addTeam}>
              <Icon name="Plus" size={14} /> Ajouter une équipe
            </button>
          </div>
          <div className="card-body">
            {teamsArr.length === 0 && (
              <div className="empty">Aucune équipe. Ajoute la première.</div>
            )}
            <div className="grid" style={{ gap: 10 }}>
              {teamsArr.map((t, i) => (
                <div key={t.info.id} className="card" style={{
                  background: 'var(--bg-2)', padding: 12, borderRadius: 12,
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '32px 32px 1fr 36px', gap: 10, alignItems: 'center', marginBottom: 10 }}>
                    ...
                    <input type="color" value={t.info.color || '#A6F060'}
                           onChange={e => updateTeamInfo(t.info.id, { color: e.target.value })}
                           title="Couleur de l'équipe"
                           style={{ width: 28, height: 28, padding: 0, background: t.info.color, border: '1px solid rgba(255,255,255,0.06)', borderRadius: 6, cursor: 'pointer' }} />
                    <input value={t.info.name} onChange={e => updateTeamInfo(t.info.id, { name: e.target.value })}
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 10 }}>
                    ...
                    <div className="field">
                      ...
                      <input type="number" min="1" max="30" className="mono"
                    ...
                    <div className="field">
                      ...
                      <input type="text" inputMode="numeric" maxLength={8} className="mono"
                             value={t.info.pin || ''}
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 10 }}>
                    <div className="field">
                      ...
                      <input type="text" value={t.info.contactName || ''}
                    ...
                    <div className="field">
                      ...
                      <input type="tel" value={t.info.contactPhone || ''}
                  ...
                  <div className="hint" style={{ marginTop: 8, display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                    ...
                    {t.info.contactName && <span>· réf. {t.info.contactName}{t.info.contactPhone ? ` · ${t.info.contactPhone}` : ''}</span>}
                  </div>
                </div>
              ))}
            </div>
